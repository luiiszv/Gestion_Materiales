...
const EditarMaterial = () => {
    ...
    const { obtenerOneMaterial, actualizarMaterial } = useAuth()
    const { id } = useParams();
    ...
    useEffect(() => {
        const material = async () => {
            const res = await obtenerOneMaterial(id)
            const { data } = res;

            console.log(data)

            if (res.status == 200) {
                setValue('nombreMaterial', data.nombre)
                setValue('descripcion_material', data.descripcion_material)
    ...
    const onSubmit = handleSubmit(async (values) => {
        ...
        const res = await actualizarMaterial(id, values);
        ...
        <div className="flex justify-center items-center h-screen ">
            <div className="w-full max-w-screen-lg xl:grid grid-cols-2 gap-4 p-6 bg-white rounded-lg shadow-lg border">
                ...
                <form onSubmit={onSubmit} className="flex flex-col">
                    <h1 className="font-bold text-center text-3xl mb-6">Editar Material</h1>
